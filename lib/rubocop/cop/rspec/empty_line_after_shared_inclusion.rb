      ...
      class EmptyLineAfterSharedInclusion < Base
        ...
        def check_inclusions(body)
          body.each_child_node do |child|
            next unless inclusion?(child)
            next if consecutive_inclusion?(child) || last_child?(child)

            missing_separating_line(child) do |location|
        ...
        def consecutive_inclusion?(node)
          ...
          sibling && inclusion?(sibling)
        end

        def inclusion?(node)
          include?(node) || conditional_inclusion?(node)
        end

        def conditional_inclusion?(node)
          return false unless node.if_type? && node.modifier_form?

          node.branches.any? { |branch| branch && include?(branch) }
        end
      end
    end
  end
