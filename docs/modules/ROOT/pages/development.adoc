...
[modify] lib/rubocop/cop/rspec.rb - `register_cop :CopName, "#{__dir__}/rspec/cop_name"` was injected.
...
The `lib/rubocop/cop/rspec/mixin/` directory contains a set of reusable modules that cops can include. The modules are autoloaded on demand; a new module needs an `autoload` entry in `lib/rubocop/cop/rspec/mixin.rb`.
