...
RSpec.describe RuboCop::Cop::RSpec::RepeatedExample do
  ...
  it 'shows line numbers for real-world scenario from GitHub issue' do
    expect_offense(<<~RUBY)
      describe 'OrderTagsService' do
        let(:contingency_date) { '2024-06-25' }
        let(:order) { create(:order) }

        context 'given an appointment-based order' do
          context 'with no availabilities' do
            it { expect(subject).not_to be_urgent(order, now: T("\#{contingency_date}T12:00-0700")) }
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Don't repeat examples within an example group. Repeated on line(s) 8.
            it { expect(subject).not_to be_urgent(order, now: T("\#{contingency_date}T12:00-0700")) }
            ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Don't repeat examples within an example group. Repeated on line(s) 7.
          end
        end
      end
    RUBY
